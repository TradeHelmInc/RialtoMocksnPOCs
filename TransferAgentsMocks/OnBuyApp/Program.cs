    ...
    class Program : ILogger
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                logger.DoLog(string.Format("OnBuy Service successfully initialized at {0}", holdingServiceURL), MessageType.Information);

                Console.ReadKey();
            }
            catch (Exception ex)
            {
                logger.DoLog(string.Format("Critical error initializing OnBuy Service:{0}", ex.Message), MessageType.Error);
            }
        }
    }
}
