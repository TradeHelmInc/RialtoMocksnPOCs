...
namespace OnOrderCancelledOrExpired
{
    class Program : ILogger
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                logger.DoLog("Initializing OnOrderCancelledOrExpired Service", MessageType.Information);

                HoldingsService holdingsService = new HoldingsService(tradingCS, orderCS, kcxURL, holdingServiceURL, logger);

                holdingsService.Run();

                logger.DoLog(string.Format("OnOrderCancelledOrExpired Service successfully initialized at {0}", holdingServiceURL), MessageType.Information);

                Console.ReadKey();
            }
            catch (Exception ex)
            {
                logger.DoLog(string.Format("Critical error initializing OnOrderCancelledOrExpired Service:{0}", ex.Message), MessageType.Error);
