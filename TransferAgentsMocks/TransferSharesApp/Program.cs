    ...
    class Program : ILogger
    {
        ...
        static void Main(string[] args)
        {
             ...
             try
            {
                ...
                logger.DoLog("Initializing Transfer Service", MessageType.Information);

                TransferService transService = new TransferService(tradingCS, orderCS, kcxURL, transferServiceURL, logger);

                transService.Run();

                logger.DoLog(string.Format("TransferShares Service successfully initialyzed at {0}", transferServiceURL), MessageType.Information);

                Console.ReadKey();
            }
            catch (Exception ex)
            {
                logger.DoLog(string.Format("Critical error initializing Transfer Service:{0}", ex.Message), MessageType.Error);
            }
        }
    }
}
