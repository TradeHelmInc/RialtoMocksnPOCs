...
namespace Rialto.LogicLayer.Builders
{
    public class SolidusToRialtoBuilder
    {
        ...
        public static Account BuildAccountFromSolidus(Rialto.Solidus.Common.DTO.Shareholders.Shareholder solidusSh,
                                                      Rialto.BusinessEntities.Shareholder shareholder,Rialto.BusinessEntities.User user)
        {
            ...
            acc.AccountNumber = Account._ACCOUNT_PREFIX + (shareholder.FirmAccountNumber != null ? shareholder.FirmAccountNumber : shareholder.FirmTaxId);
            acc.AccountName = Account._ACCOUNT_PREFIX + " " + shareholder.Name;
            acc.AccountStatus = true;
            acc.SettlementInstructions = null;
            acc.ClientContact = solidusSh.GetAddress();
            acc.Email = solidusSh.emailAddress;
            acc.PhoneNumber = solidusSh.GetPhone();
            acc.AccountType = null;
            acc.ActivationDate = DateTime.Now;
            acc.AddressStreet = solidusSh.residenceStreetAddress;
            acc.AddressCity = solidusSh.residenceCity;
            acc.AddressState = solidusSh.residenceState;
            acc.AddressCountry = solidusSh.residenceCountry;
            acc.AddressZipCode = solidusSh.residencePostalCode;
            acc.TaxDomicile = null;
            acc.TaxFormType = null;
            acc.TaxFormStatus = null;
            acc.SSNTinEin = null;
            acc.PepStatus = null;
            acc.InsiderStatus = null;
            acc.FinraRegistration = null;
            acc.FeeMatrix = null;
            acc.TopAccount = null;
            acc.Shareholder = shareholder;
            acc.UserCount = 1;
            acc.SecFees = null;
            acc.Commission = null;
            acc.DefaultUser = user;
